            ...
            if (serviceCount == 0) {
                ...
                for (i in createdServices.indices) {
                    ...
                    if (i > 0) {
                        ...
                        // Parse the ID to UUID before using it
                        val serviceId = if (service.id != null && service.id.isNotEmpty()) {
                            Uuid.parse(service.id)
                        } else {
                            logger.warn("Service ${service.title} has no ID, skipping update")
                            continue
                        }

                        val updated = serviceService.updateService(serviceId, updateRequest)
                        if (updated != null) {
