...
class UserMapper {
    ...
    fun toEntity(user: User): UserEntity {
        ...
        entity.createdAt = user.createdAt?.let { parseToJavaLocalDateTime(it) } ?: java.time.LocalDateTime.now()
        entity.updatedAt = user.updatedAt?.let { parseToJavaLocalDateTime(it) } ?: java.time.LocalDateTime.now()
        entity.lastLogin = user.lastLogin?.let { parseToJavaLocalDateTime(it) }
    ...
    private fun parseToJavaLocalDateTime(dateStr: String): java.time.LocalDateTime {
        return try {
            java.time.LocalDateTime.parse(dateStr, dateFormatter)
        } catch (e: Exception) {
